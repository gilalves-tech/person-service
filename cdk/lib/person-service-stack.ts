...
export class PersonServiceStack extends Stack {
	...
	private initialiseCreatePersonLambda(): void {
		...
		this.createPersonLambda = new LambdaResource(this, 'CreatePersonLambdaResource', this.config, {
			functionName: `${LambdaFunctionNames.CREATE_PERSON}-${this.config.stage}`,
			handler: "handler",
			entry: "src/handlers/create-person.ts",
			role: createPersonIam.role,
			environment: {
				SNS_TOPIC_ARN: this.snsResource.topicArn,
				...this.commonEnvironmentVariables
			}
	...
	private initialiseListPersonsLambda(): void {
		const listPersonsIam = new IamResource(this, `ListPersonsIamResource`, this.config, `${LambdaFunctionNames.LIST_PERSONS}-role`);

		this.listPersonsLambda = new LambdaResource(this, 'ListPersonsLambdaResource', this.config, {
			functionName: `${LambdaFunctionNames.LIST_PERSONS}-${this.config.stage}`,
			handler: "handler",
			entry: "src/handlers/list-persons.ts",
			role: listPersonsIam.role,
			environment: {
				...this.commonEnvironmentVariables
			}
		});
